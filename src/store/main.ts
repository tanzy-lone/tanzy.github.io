...
export const useMainStore = defineStore('mainStore', {
	...
	actions: {
		...
		hasTabs(payload: any) {
			return this.filterRoute.every((item: any) => item.url !== payload.url)
		},
		changeFilterRoute(payload: any) {
			if (this.hasTabs(payload)) {
				console.log('payload', payload)
				if (payload.name === 'home' && !payload.isClose) {
					payload.close = false
				} else {
					payload.close = true
				}
		...
		removeTabsAction(payload: TabPaneName, isCurrRoute: boolean) {
			let i: number = 0
			const removeList = this.filterRoute.filter((item: IFlatMenu, index: number) => {
				if (item.url === payload) {
					i = index
				}
			...
			if (isCurrRoute) {
				router.push(this.filterRoute[i - 1].url)
			}
			this.filterRoute = removeList
		},
		setTabsListAction(tabsList: IFlatMenu[]) {
			this.filterRoute = tabsList
		}
	},
	persist: {
